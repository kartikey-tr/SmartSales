...
class CustomersViewModel @Inject constructor(
    ...
    fun saveCustomer(name: String, phone: String, email: String, address: String) {
        ...
        viewModelScope.launch {
            ...
            if (existing != null) {
                customerDao.updateCustomer(existing.copy(name = name.trim(), phone = phone.trim(), email = email.trim(), address = address.trim()))
            } else {
                customerDao.insertCustomer(CustomerEntity(name = name.trim(), phone = phone.trim(), email = email.trim(), address = address.trim()))
            }
            closeDialog()
        }
    }
